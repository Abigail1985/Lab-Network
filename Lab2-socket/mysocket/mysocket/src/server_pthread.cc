...
#define BUFFER_SIZE                61440         //61440=1024*60, make buffer=480kB
...
void *single_file_request(void *socket_)
{

    pthread_mutex_lock(&mutex);

    int *server_socket_connect=(int*)socket_;
    // memcpy(&*server_socket_connect, (void *)&socket_, 4); 失败
    //强制类型转换会报错“cast from ‘void*’ to ‘int’ loses precision“

    printf("Successfully connect with one client with socket NO.%d...\n",*server_socket_connect);//必须要带*，否则打印地址


    char buffer[BUFFER_SIZE];
    bzero(buffer, sizeof(buffer));//初始化
    socklen_t length = recv(*server_socket_connect, buffer, BUFFER_SIZE, 0);
    ...
    else
    {
...

        {
            ...
            // 发送buffer中的字符串到*server_socket_connect,实际上就是发送给客户端
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        // 这里的*server_socket_connect代表了这个通信通道
        ...
        if(pthread_create(&thread_id,NULL,&single_file_request,&server_socket_connect))
        {
        ...
        // {//连接线程，thread_id是被连接的线程号，当主线程需要在子线程结束后继续时，使用join，成功的
